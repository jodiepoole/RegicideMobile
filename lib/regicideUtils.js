import { SUITS, TAVERN_SUIT_CARDS, ENEMIES, HAND_LIMIT, ANIMAL_COMPANION, SPADES, HEARTS, DIAMONDS, CLUBS, CLUBS_DAMAGE_MODIFIER, BOSS_DAMAGE } from "../lib/constants";
...
export const sumCardValues = (cards) => cards.reduce((accumulator, object) => {
    const value = isNaN(object.value) ? BOSS_DAMAGE[object.value] : object.value;
...
export const validateCombo = (selectedCard, card, sum) => {
    return ( 
        (card.value !== ANIMAL_COMPANION && selectedCard.value !== ANIMAL_COMPANION) && (selectedCard.value !== card.value || (sum + card.value) > 10 || isNaN(card.value))
    );
}